...
- Files are automatically base64-encoded during transfer
   `download <remote_file_path> [agent_id]` - queues download task for the agent
...
- `upload <local_file_path> <remote_file_path> [agent_id]` - agent receives and decodes the file
