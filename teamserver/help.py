...
SYNTAX:
  • stager <list|generate> [options]

COMMANDS:
  • stager list                               List all stagers
  • stager generate <type> [options]          Generate new interactive stager

EXAMPLES:
  • stager list
  • stager generate python host=192.168.1.10 port=443 protocol=https
  • stager generate bash host=10.0.0.50 port=80 protocol=http

TYPES:
  • python, bash, powershell, exe, dll, js

TIPS:
  • Use 'stager list' to see available stager types
  • Configure host, port, and protocol parameters as needed
...
def get_save_help_display():
    return """\nFILE SAVING & LOGGING - Command Reference
═════════════════════════════════════════════════════════════════════════════
