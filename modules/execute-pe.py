...
def get_info():
    return {
        "name": "execute-pe",
        "description": "Execute a PE file in memory on the target agent using goffloader for in-memory execution without touching disk",
        "type": "execution",
        "platform": "windows",
        ...
        "options": {
            "agent_id": {
                "description": "ID of the agent to execute PE on",
                "required": True
            },
            "pe_file": {
                "description": "Path to the PE file to execute in memory on client machine",
                "required": True
            },
            "arguments": {
                "description": "Arguments to pass to the PE file (optional)",
                "required": False,
                "default": ""
            }
        },
        "notes": {
            "description": "Executes PE files in memory using goffloader, bypassing traditional file-based execution methods"
        }
    }
...
def execute(options, session):
    ...
    # Prepare the command with arguments if provided
    if arguments:
        command = f"execute-pe {prefixed_encoded_pe} {arguments}"
    else:
        command = f"execute-pe {prefixed_encoded_pe}"
    ...
    if hasattr(session, 'is_interactive_execution') and session.is_interactive_execution:
        ...
        return {
            "success": True,
            "output": f"[x] PE execution prepared for interactive mode",
            "command": command,
        }
        ...
        try:
            ...
            if task_id:
                return {
                    "success": True,
                    "output": f"[x] PE execution task {task_id} queued for agent {agent_id}",
                    "task_id": task_id,
                }
            else:
                return {
